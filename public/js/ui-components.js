...
const UI = {
 ...
 progress(value,label=''){
  ...
  return `<div class="project-progress">
   <div class="project-progress-track"><i style="width:${safe}%"></i></div>
   ${label?`<div class="project-progress-label">${this.escape(label)}</div>`:''}
  </div>`;
 },
 button(label,onClick,variant='primary'){
  return `<button class="project-button project-button-${variant}" onclick="${onClick}">
   <span>${this.escape(label)}</span>${this.icon('arrow',18)}
  </button>`;
 }
};
